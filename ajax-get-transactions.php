...
if ( !isset($location) ) {
	...
	foreach($transactionResults as $record) {
		...
		$customerSignature = $qbCustomers->do_query($customersQueries, '', '', '81', '', 'structured', 'sortorder-A');
		...
		// do the query in the CUSTOMERS table for thumbprint
		$customerThumbprint = $qbCustomers->do_query($customersQueries, '', '', '80', '', 'structured', 'sortorder-A');
		$customerThumbprint = $customerThumbprint->table->records->record->f;
		$customerThumbprint = json_decode(json_encode($customerThumbprint), true);
